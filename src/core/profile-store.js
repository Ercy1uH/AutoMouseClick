...
const { validateTree, metrics, flatten } = require('../renderer/point-settings');
const SCHEMA_VERSION = 5, MIN_SCHEMA_VERSION = 1, MAX_PROFILES = 8, MAX_LABEL = 80;
...
function sanitizeProfile(profile, index, strict = false) {
  ...
  const modern = source.some(step => step?.id !== undefined || step?.type === 'loop' || (step?.type && !['click', 'delay'].includes(step.type)));
  const steps = validateTree(strict || modern ? source : limitSteps(source.map((step) => sanitizeStep(step, fallbackClickType, legacyInterval(profile.pointInterval))).filter(Boolean)), { fallbackClickType });
...
function pointClicks(step) { return step?.type === 'loop' ? metrics(step.steps).clicks * step.repeatCount : step?.type === 'delay' ? 0 : requireStepSettings(step).clickCount; }
function totalClicks(steps, loops) { const total = (Array.isArray(steps) ? steps : []).reduce((sum, step) => sum + pointClicks(step), 0) * clampNumber(loops, 1, MAX_LOOPS, 1); if (!Number.isSafeInteger(total)) throw new Error('总点击动作超出安全整数范围'); return total; }
class ProfileStore {
  ...
  write() { fs.mkdirSync(path.dirname(this.file), { recursive: true }); const temp = `${this.file}.tmp`; try { fs.writeFileSync(temp, JSON.stringify({ schemaVersion: SCHEMA_VERSION, active: this.active, profiles: this.profiles }), { encoding: 'utf8', flush: true }); if (this.migrationVersion) this.backupOriginal(); fs.renameSync(temp, this.file); this.migrationVersion = null; } catch (error) { try { fs.unlinkSync(temp); } catch {} throw error; } }
  // 与 RunHistory.backupOriginal 同一判据：同名备份必须逐字节等于本次原文件才可复用，
  // 否则换一个不冲突的名字再留一份；一份都留不下就抛错，绝不替换原文件。
  backupOriginal() {
    const directory = path.dirname(this.file); const original = fs.readFileSync(this.file);
    for (let attempt = 0; attempt < 100; attempt += 1) {
      const name = attempt === 0 ? `profiles.v${this.migrationVersion}.backup.json` : `profiles.v${this.migrationVersion}.${attempt}.backup.json`;
      const target = path.join(directory, name);
      try { fs.copyFileSync(this.file, target, fs.constants.COPYFILE_EXCL); return target; }
      catch (error) { if (error.code !== 'EEXIST') throw error; if (fs.readFileSync(target).equals(original)) return target; }
    }
    throw new Error('无法为配置迁移创建备份');
  }
}
