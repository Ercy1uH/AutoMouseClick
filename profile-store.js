...
const { requireStepSettings, sanitizeStepSettings, resolveLabelAuto, pointsToSteps, legacyInterval, clickType, applyAutoLabels, MAX_POINT_CLICKS, MAX_STEPS, MAX_CLICK_STEPS, DEFAULT_CLICK_TYPE } = require('./point-settings');
...
function sanitizeProfile(profile, index, strict = false) {
  ...
  return { name, note: typeof profile.note === 'string' ? profile.note.slice(0, 120) : `${clickCount} 个坐标点`, steps, defaultClickType: fallbackClickType, loops: clampNumber(profile.loops, 1, 100000, 1), loopInterval: clampNumber(profile.loopInterval, 0, 60000, 500) };
}
...
function totalClicks(steps, loops) { const rounds = clampNumber(loops, 1, 100000, 1); return (Array.isArray(steps) ? steps : []).reduce((sum, step) => sum + pointClicks(step), 0) * rounds; }
class ProfileStore {
  constructor(file) { this.file = file; this.profiles = []; this.active = 0; this.error = null; try { const data = JSON.parse(fs.readFileSync(file, 'utf8')); if (!Number.isInteger(data.schemaVersion) || data.schemaVersion < MIN_SCHEMA_VERSION || data.schemaVersion > SCHEMA_VERSION || !Array.isArray(data.profiles)) throw new Error('Invalid profile store'); this.profiles = sanitizeProfiles(data.profiles); this.schemaVersion = SCHEMA_VERSION; this.active = sanitizeActive(data.active, this.profiles); if (data.schemaVersion < SCHEMA_VERSION) { this.migrationVersion = data.schemaVersion; try { this.write(); } catch { this.error = '配置升级失败，原配置未修改'; } } } catch (error) { if (error.code !== 'ENOENT') this.error = '配置读取失败，已回退默认配置'; } }
